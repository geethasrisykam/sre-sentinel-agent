  ...
  return (
    <div className="glass-card p-5">
      ...
      <ol className="mt-4 space-y-3">
        {turns.map((turn, idx) => (
          <li key={idx} className="relative pl-6 animate-slide-up">
            <span className="absolute left-0 top-1.5 h-2 w-2 rounded-full bg-cyan-500 dark:bg-cyan-500" />
            {idx < turns.length - 1 && (
              <span className="absolute left-[3px] top-3.5 h-full w-px bg-slate-200 dark:bg-slate-800" />
            )}
            <div className="flex items-center gap-2">
              ...
              {turn.toolCall && (
                <span className="rounded bg-cyan-100 px-1.5 py-0.5 font-mono text-[10px] text-cyan-700 dark:bg-cyan-950/60 dark:text-cyan-300">
                  {turn.toolCall.name}
                </span>
              )}
            </div>
            <div className="mt-1 text-sm text-slate-800 dark:text-slate-200">{turn.thought}</div>
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1.5">
      {entries.map(([key, value]) => (
        <div
          key={key}
          className="flex items-center gap-1.5 rounded border px-2 py-0.5 font-mono text-[11px] border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/40"
        >
          <span className="text-slate-500 dark:text-slate-500">{key}</span>
          <span className="text-slate-800 dark:text-slate-300">{renderArgValue(value)}</span>
        </div>
      ))}
    </div>
  );
...
function ResultDetails({ result }: { result: { ok: boolean; data: unknown } }) {
  ...
  return (
    <details className="mt-1.5">
      <summary className="cursor-pointer text-[11px] text-slate-500 hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-300">
        <span
          className={
            result.ok
      ...
      <pre className="mt-1 max-h-64 overflow-auto rounded border p-2 font-mono text-[11px] border-slate-200 bg-slate-50 text-slate-700 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-400">
